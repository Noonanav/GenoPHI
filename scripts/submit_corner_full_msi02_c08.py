#!/usr/bin/env python3
"""
FULL-DATASET leave-one-genus-out CORNER run at the winning config from the
subset threshold scan: min_seq_id=0.2, coverage=0.8 (best genus-mean AUC).
...
Single combo (NOT a grid):
    1 shared-clustering job  ->  fold corner array (afterok)  ->  aggregate (afterok)

Fold inputs: the FULL corner folds built by build_corner_folds.py from
combined_interactions_full_4genera.csv -- one fold_<genus>/ per genus with
training/validation x strains/phages CSVs (+ held_out_group.txt).

  python submit_corner_full_msi02_c08.py                    # all genera (full run)
  python submit_corner_full_msi02_c08.py --test-fold fold_Vibrio   # one genus
"""
import os
import argparse
import subprocess
...
base_output_dir = "/global/scratch/users/anoonan/set_transformer/manuscript/genophi_corner_FULL_msi02_c08"

# FULL run at the winning config from the subset scan (best genus-mean): the
# single combo min_seq_id=0.2, coverage=0.8. NOT a grid.
MIN_SEQ_IDS = [0.2]
COVERAGES = [0.8]
