#!/bin/bash
#SBATCH --job-name=b2b_curated_pub
...
WORK=/global/scratch/users/anoonan/BRaVE/struct_cv/curated_published
CURATED_FAA="$WORK/curated_published.faa"      # scp'd from niya
OUT="$WORK"
mkdir -p "$OUT"
...
open(out,"w").write("\n".join(sorted(reps))+"\n")
print(f"curated-published -> {len(reps)} clusters (unmapped {un})", file=sys.stderr)
PY
fi
...
for i in $(seq -w 1 16); do
  ...
  dst="$OUT/${f}_curatedpub_3metric.csv"
