...
Handles both main workflow and multiple kmer_modeling workflows (kmer_modeling_k) from all iterations.
"""
...
def process_single_kmer_workflow(kmer_src, kmer_dest, kmer_name, minimal=False):
    """
    Process a single kmer_modeling workflow directory.
    
    Args:
        kmer_src (Path): Source kmer_modeling directory
        kmer_dest (Path): Destination kmer_modeling directory  
        kmer_name (str): Name of the kmer directory (e.g., "kmer_modeling_3")
        minimal (bool): If True, skip large files and only copy best cutoff
    ...
    print(f"  Processing {kmer_name} workflow...")
    files_copied = 0
    ...
    for filename in performance_files:
        ...
        if copy_file_safe(src_file, dest_file, f"{kmer_name}/{filename}"):
            files_copied += 1
    ...
    if best_cutoff:
        ...
        if best_cutoff.startswith('cutoff_'):
            ...
            if copy_file_safe(src_file, dest_file, f"{kmer_name}/best feature table ({best_feature_file})"):
                files_copied += 1
        else:
            print(f"    ⚠ Warning: Unexpected {kmer_name} cutoff format: {best_cutoff}")
    else:
        print(f"    ⚠ Warning: Could not determine {kmer_name} best cutoff, skipping feature tables")
    ...
    if minimal:
        ...
        if copy_file_safe(kmer_final_feature_src, kmer_final_feature_dest, f"{kmer_name}/feature_tables/final_feature_table.csv"):
            files_copied += 1
    else:
        ...
        if copy_directory_safe(kmer_feature_tables_src_dir, kmer_feature_tables_dest_dir, f"{kmer_name}/feature_tables directory"):
            files_copied += 1
    ...
    if copy_file_safe(kmer_full_feature_src, kmer_full_feature_dest, f"{kmer_name}/full_feature_table.csv"):
        files_copied += 1
    
    # Copy kmer workflow_report.txt
    kmer_report_src = kmer_src / "workflow_report.txt"
    kmer_report_dest = kmer_dest / "workflow_report.txt"
    if copy_file_safe(kmer_report_src, kmer_report_dest, f"{kmer_name}/workflow_report.txt"):
        files_copied += 1
    ...
    if copy_file_safe(kmer_strain_pred_src, kmer_strain_pred_dest, f"{kmer_name}/strain_median_predictions.csv"):
        files_copied += 1
                ...
                if kmer_best_cutoff_src.exists():
                    ...
                    for run_dir in run_dirs:
                        ...
                        for filename in importance_files:
                            src_file = run_dir / filename
                            dest_file = dest_run_dir / filename
                            if copy_file_safe(src_file, dest_file, f"{kmer_name}/cutoff_{cutoff_num}/{run_name}/{filename}"):
                                kmer_cutoff_files_copied += 1
                    
                    if kmer_cutoff_files_copied > 0:
                        files_copied += kmer_cutoff_files_copied
                        print(f"    ✓ Copied {kmer_cutoff_files_copied} {kmer_name} importance files from cutoff_{cutoff_num}")
    
    return files_copied


def process_kmer_workflows(iteration_dir, dest_iteration, minimal=False):
    """
    Process all kmer_modeling workflows for a single iteration.
    ...
    Returns:
        tuple: (total_files_copied, number_of_kmer_dirs_found)
    """
    # Find all kmer_modeling directories (pattern: kmer_modeling_*)
    kmer_dirs = sorted([d for d in iteration_dir.iterdir() 
                       if d.is_dir() and d.name.startswith('kmer_modeling_')])
    ...
    for kmer_dir in kmer_dirs:
        kmer_name = kmer_dir.name
        kmer_src = kmer_dir
        kmer_dest = dest_iteration / kmer_name
        
        files_copied = process_single_kmer_workflow(kmer_src, kmer_dest, kmer_name, minimal)
        total_files_copied += files_copied
    
    return total_files_copied, len(kmer_dirs)


def export_bootstrap_results(source_dir, dest_dir, minimal=False):
    """
    Export bootstrap validation results including both main and all kmer_modeling workflows.
    ...
    for iteration_dir in iteration_dirs:
        ...
        # Process all kmer_modeling workflows if they exist
        kmer_files, num_kmer_dirs = process_kmer_workflows(iteration_dir, dest_iteration, minimal)
        if num_kmer_dirs > 0:
            kmer_iterations_found += 1
            total_kmer_dirs_found += num_kmer_dirs
        ...
        if iteration_files_copied > 0:
            ...
            print(f"  ✓ Copied {iteration_files_copied} items from {iteration_name} (main: {main_files}, kmer: {kmer_files} from {num_kmer_dirs} kmer dirs)")
        else:
    ...
    print(f"Total kmer_modeling directories found: {total_kmer_dirs_found}")
    ...
    create_manifest(dest_dir, source_dir, successful_iterations, total_iterations, kmer_iterations_found, total_kmer_dirs_found, minimal)


def create_manifest(dest_dir, source_dir, successful_iterations, total_iterations, kmer_iterations_found, total_kmer_dirs_found, minimal=False):
...
Total kmer_modeling directories: {total_kmer_dirs_found}
...
KMER_MODELING WORKFLOWS (multiple k-values, if present):
For each kmer_modeling_X directory found:
- kmer_modeling_X/modeling/modeling_results/model_performance/model_performance_metrics.csv
- kmer_modeling_X/modeling/modeling_results/model_performance/pr_curve.png
- kmer_modeling_X/modeling/modeling_results/model_performance/roc_curve.png
- kmer_modeling_X/modeling/feature_selection/filtered_feature_tables/select_feature_table_cutoff_X.csv (best cutoff only)
- kmer_modeling_X/modeling/modeling_results/select_features_model_performance.csv
- kmer_modeling_X/modeling/modeling_results/select_features_model_predictions.csv"""

    if not minimal:
        manifest_content += """
- kmer_modeling_X/modeling/modeling_results/cutoff_[BEST]/run_*/[shap_importances.csv, feature_importances.csv]
- kmer_modeling_X/feature_tables/ (directory with feature assignments)"""
    else:
        manifest_content += """
- kmer_modeling_X/feature_tables/final_feature_table.csv (only this file)
  (Note: kmer cutoff directories excluded in ultra-minimal mode)"""

    manifest_content += """
- kmer_modeling_X/model_validation/predict_results/strain_median_predictions.csv (if present)
- kmer_modeling_X/full_feature_table.csv
- kmer_modeling_X/workflow_report.txt
...
each bootstrap validation iteration for both main and all kmer_modeling workflows.
Each kmer_modeling_X directory represents results for a different k-mer size (X).
...
- Includes complete kmer_modeling results for all k-values
...
- kmer_modeling_X/modeling/modeling_results/ completely excluded
- kmer_modeling_X/feature_tables/ reduced to only final_feature_table.csv
- kmer_modeling_X/model_validation/ is KEPT (contains important predictions)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export bootstrap validation results from HPC cluster (main + multiple kmer_modeling)",
