#!/usr/bin/env python3
"""
SLURM workflow submission for strain+phage-based bootstrap cross-validation.
Creates a job array where each job handles one iteration of the bootstrap validation.
This version performs cross-validation by leaving out BOTH 10% of strains AND 10% of phages,
testing generalization to completely novel strain-phage combinations.
"""
...
def create_bootstrap_job_array(args, run_dir):
    """Create SLURM job array script for bootstrap iterations - STRAIN+PHAGE-based CV"""
    
    # Get the absolute path to the original script directory for imports
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    script_content = f"""#!/bin/bash
#SBATCH --job-name=bootstrap_strain_phage_cv
...
#SBATCH --output=logs/bootstrap_strain_phage_cv_%A_%a.out
#SBATCH --error=logs/bootstrap_strain_phage_cv_%A_%a.err

echo "=== Bootstrap Strain+Phage-based Cross-Validation - Iteration $SLURM_ARRAY_TASK_ID ==="
echo "Job: $SLURM_JOB_ID, Array Task: $SLURM_ARRAY_TASK_ID, Node: $SLURMD_NODENAME, Started: $(date)"
echo "CV Type: Strain+Phage-based (training on 90% strains + 90% phages, validating on 10% strains + 10% phages)"
...
def get_full_strain_list(interaction_matrix, input_strain_dir, strain_column):
    '''Get list of strains that exist in both interaction matrix and strain directory'''
...
def split_strains_and_phages(full_strain_list, full_phage_list, iteration, validation_percentage=0.1):
    '''Split both strains and phages into modeling and validation sets'''
    if len(full_strain_list) == 0 or len(full_phage_list) == 0:
        print('ERROR: Cannot split empty strain or phage lists!')
        return [], [], [], []
        
    print(f'Splitting {{len(full_strain_list)}} strains and {{len(full_phage_list)}} phages for iteration {{iteration}}')
    
    # Use the same seed for both splits to ensure reproducibility
    random.seed(iteration)
    
    # Split strains
    strain_list_copy = full_strain_list.copy()
    random.shuffle(strain_list_copy)
    strain_split_index = int(len(strain_list_copy) * (1 - validation_percentage))
    modeling_strains = strain_list_copy[:strain_split_index]
    validation_strains = strain_list_copy[strain_split_index:]
    
    # Split phages (re-seed with iteration+1000 to get different split)
    random.seed(iteration + 1000)
    phage_list_copy = full_phage_list.copy()
    random.shuffle(phage_list_copy)
    phage_split_index = int(len(phage_list_copy) * (1 - validation_percentage))
    modeling_phages = phage_list_copy[:phage_split_index]
    validation_phages = phage_list_copy[phage_split_index:]
    ...
    return modeling_strains, validation_strains, modeling_phages, validation_phages

def create_modeling_interaction_matrix(interaction_matrix, modeling_strains, modeling_phages, iteration_output_dir):
    '''Create interaction matrix filtered to modeling strains AND modeling phages only'''
    interaction_df = pd.read_csv(interaction_matrix)
    
    # Filter to only interactions between modeling strains and modeling phages
    modeling_interaction_df = interaction_df[
        (interaction_df['strain'].isin(modeling_strains)) &
        (interaction_df['phage'].isin(modeling_phages))
    ]
    ...
    original_interactions = len(interaction_df)
    modeling_interactions = len(modeling_interaction_df)
    retention_rate = (modeling_interactions / original_interactions) * 100
    
    print(f'Created modeling interaction matrix with {{modeling_interactions}} interactions ({{retention_rate:.1f}}% of original {{original_interactions}})')
...
def create_validation_interaction_matrix(interaction_matrix, validation_strains, validation_phages, iteration_output_dir):
    '''Create interaction matrix for validation (validation strains × validation phages only)'''
    interaction_df = pd.read_csv(interaction_matrix)
    
    # Filter to only interactions between validation strains and validation phages
    validation_interaction_df = interaction_df[
        (interaction_df['strain'].isin(validation_strains)) &
        (interaction_df['phage'].isin(validation_phages))
    ]
    
    validation_interaction_path = os.path.join(iteration_output_dir, 'validation_interaction_matrix.csv')
    validation_interaction_df.to_csv(validation_interaction_path, index=False)
    
    print(f'Created validation interaction matrix with {{len(validation_interaction_df)}} interactions for validation')
    return validation_interaction_path
...
if len(full_strain_list) == 0 or len(full_phage_list) == 0:
    print('FATAL ERROR: No valid strains or phages found. Cannot proceed with iteration.')
    sys.exit(1)

# Handle clustering directory and strain/phage splits
...
if clustering_dir:
    ...
    if not all(os.path.exists(p) for p in [modeling_strains_path, validation_strains_path, modeling_phages_path, validation_phages_path]):
        modeling_strains, validation_strains, modeling_phages, validation_phages = split_strains_and_phages(
            full_strain_list, full_phage_list, iteration=iteration
        )
        
        if any(len(lst) == 0 for lst in [modeling_strains, validation_strains, modeling_phages, validation_phages]):
            print(f'ERROR: Empty lists created for iteration {{iteration}}')
        ...
        test_modeling_strains = pd.read_csv(modeling_strains_path)
        test_validation_strains = pd.read_csv(validation_strains_path)
        test_modeling_phages = pd.read_csv(modeling_phages_path)
        test_validation_phages = pd.read_csv(validation_phages_path)
        print(f'Verification - Strain CSVs: {{len(test_modeling_strains)}} + {{len(test_validation_strains)}} rows')
        print(f'Verification - Phage CSVs: {{len(test_modeling_phages)}} + {{len(test_validation_phages)}} rows')
        
    else:
        print('Strain and phage lists already exist. Loading...')
        ...
        if any(len(df) == 0 for df in [modeling_strains_df, validation_strains_df, modeling_phages_df, validation_phages_df]):
            print(f'ERROR: Existing CSV files are empty!')
...
# Create filtered interaction matrix for modeling (modeling strains × modeling phages only)
modeling_interaction_matrix = create_modeling_interaction_matrix(
    '{args.interaction_matrix}', modeling_strains, modeling_phages, iteration_output_dir
)

# Step 1: Run protein_family_workflow with modeling strains × modeling phages
...
if not os.path.exists(metrics_file):
    print('Running protein family workflow with modeling strains × modeling phages...')
    run_protein_family_workflow(
        ...
        phage_column='phage',
        strain_list=os.path.join(iteration_output_dir, 'modeling_strains.csv'),  # Filter to modeling strains
...
# Step 3: Assign features to validation strains and validation phages
...
# Check for modified AA directories
modified_strain_aa_dir = os.path.join(iteration_output_dir, 'strain', 'modified_AAs', 'strain')
input_strain_dir = modified_strain_aa_dir if os.path.exists(modified_strain_aa_dir) else '{args.input_strain_dir}'

modified_phage_aa_dir = os.path.join(iteration_output_dir, 'phage', 'modified_AAs', 'phage')
input_phage_dir = modified_phage_aa_dir if os.path.exists(modified_phage_aa_dir) else '{args.input_phage_dir}'
...
print('Step 3a: Assigning features to validation strains...')
# Assign features to validation strains using training strain clustering
strain_assign_output_dir = os.path.join(validation_output_dir, 'strain_assign_results')
run_assign_features_workflow(
    input_dir=input_strain_dir,  # Validation strain directory 
    mmseqs_db=os.path.join(iteration_output_dir, 'tmp', 'strain', 'mmseqs_db'),  # Strain database from training
    tmp_dir=os.path.join(validation_tmp_dir, 'strain'),
    output_dir=strain_assign_output_dir,
    feature_map=os.path.join(iteration_output_dir, 'strain', 'features', 'selected_features.csv'),  # Strain feature mapping
    clusters_tsv=os.path.join(iteration_output_dir, 'strain', 'clusters.tsv'),  # Strain clusters from training  
    genome_type='strain',  # Assigning features TO strains
    genome_list=os.path.join(iteration_output_dir, 'validation_strains.csv'),  # Only validation strains
    sensitivity=7.5,
    coverage=0.8,
    min_seq_id=0.4,
    threads={args.threads},
...
print('Step 3b: Assigning features to validation phages...')
# Assign features to validation phages using training phage clustering
phage_assign_output_dir = os.path.join(validation_output_dir, 'phage_assign_results')
run_assign_features_workflow(
    ...
    tmp_dir=os.path.join(validation_tmp_dir, 'phage'),
    output_dir=phage_assign_output_dir,
...
print('Step 3c: Predicting interactions between validation strains and validation phages...')
# Now predict using assigned strain features + assigned phage features (both validation)
validation_strain_features_path = os.path.join(strain_assign_output_dir, 'strain_combined_feature_table.csv')
validation_phage_features_path = os.path.join(phage_assign_output_dir, 'phage_combined_feature_table.csv')

# Verify that both feature files exist
if not os.path.exists(validation_strain_features_path):
    print(f'ERROR: Validation strain features not found at {{validation_strain_features_path}}')
    sys.exit(1)
if not os.path.exists(validation_phage_features_path):
    print(f'ERROR: Validation phage features not found at {{validation_phage_features_path}}')
    sys.exit(1)

print(f'Using validation strain features: {{validation_strain_features_path}}')
print(f'Using validation phage features: {{validation_phage_features_path}}')

# Create properly named input directory for prediction workflow
...
if not os.path.exists(strain_features_for_prediction):
    shutil.copy2(validation_strain_features_path, strain_features_for_prediction)
    print(f'Copied validation strain features for prediction workflow')

# Verify feature table dimensions
strain_features_df = pd.read_csv(strain_features_for_prediction)
phage_features_df = pd.read_csv(validation_phage_features_path)
print(f'Validation strain features: {{len(strain_features_df)}} strains, {{len(strain_features_df.columns)}} columns')
print(f'Validation phage features: {{len(phage_features_df)}} phages, {{len(phage_features_df.columns)}} columns')
print(f'Will predict {{len(strain_features_df) * len(phage_features_df)}} strain-phage interactions')

predict_output_dir = os.path.join(validation_output_dir, 'predict_results')

run_prediction_workflow(
    input_dir=predict_input_dir,  # Directory with validation strain features
...
print(f'Predicted {{len(validation_strains)}} validation strains × {{len(validation_phages)}} validation phages = {{len(validation_strains) * len(validation_phages)}} novel interactions')
"

echo "Iteration $SLURM_ARRAY_TASK_ID completed: $(date)"
"""
    
    script_path = os.path.join(run_dir, "bootstrap_strain_phage_cv_job_array.sh")
...
def create_final_aggregation_job(args, run_dir, dependency):
    ...
    script_content = f"""#!/bin/bash
#SBATCH --job-name=bootstrap_strain_phage_aggregate
#SBATCH --account={args.account}
#SBATCH --partition={args.partition}
#SBATCH --qos={args.qos}
#SBATCH --nodes=1
...
echo "=== Bootstrap Strain+Phage-based CV Results Aggregation ==="
...
# Aggregate final predictions from all iterations (strain+phage-based CV)
output_dir = '{args.output_dir}'
final_predictions = pd.DataFrame()

print('Aggregating results from {args.n_iterations} iterations (strain+phage-based CV)...')
print('Each iteration predicted interactions between 10% validation strains and 10% validation phages')
print('This tests generalization to completely novel strain-phage combinations')
...
if len(final_predictions) > 0:
    ...
    try:
        # Load original interaction matrix
        original_interactions_df = pd.read_csv('{args.interaction_matrix}')
        original_count = len(original_interactions_df)
        
        # Count unique validation interactions across all iterations  
        validation_count = len(summary_stats)
        retention_rate = (validation_count / original_count) * 100
        
        print(f'Original interactions: {{original_count}}')
        print(f'Validation interactions: {{validation_count}}')
        print(f'Data retention rate: {{retention_rate:.1f}}%')
        
        # Save retention statistics
        retention_stats = {{
            'original_interactions': original_count,
            'validation_interactions': validation_count, 
            'retention_rate_percent': retention_rate,
            'completed_iterations': completed_iterations,
            'total_predictions': len(final_predictions)
        }}
        retention_df = pd.DataFrame([retention_stats])
        retention_df.to_csv(os.path.join(output_dir, 'data_retention_stats.csv'), index=False)
        print(f'Data retention statistics saved')
        
    except Exception as e:
        print(f'Could not calculate retention statistics: {{e}}')
else:
...
def main():
    parser = argparse.ArgumentParser(description="Submit strain+phage-based bootstrap validation workflow as SLURM job array")
    ...
    parser.add_argument('--filter_type', type=str, default='strain', help="Filter type for cross-validation ('strain' or 'phage').")
    ...
    # Validate interaction matrix has expected columns
    try:
        import pandas as pd
        ...
        if args.phage_column not in df.columns:
            ...
            print(f"Available columns: {list(df.columns)}")
            return 1
        print(f"Interaction matrix validation: ✓ Found {len(df)} rows with columns '{args.strain_column}' and '{args.phage_column}'")
    except Exception as e:
    ...
    run_dir = f"bootstrap_strain_phage_cv_run_{timestamp}"
    os.makedirs(run_dir, exist_ok=True)
    os.makedirs(os.path.join(run_dir, "logs"), exist_ok=True)
    
    print(f"=== Strain+Phage-based Bootstrap Validation SLURM Submission ===")
    print(f"Run directory: {run_dir}")
    print(f"Output directory: {args.output_dir}")
    print(f"Cross-validation type: STRAIN+PHAGE-based (leaving out both strains AND phages)")
    ...
    bootstrap_job_id = submit_job("bootstrap_strain_phage_cv_job_array.sh")
    ...
    print(f"\n=== Strain+Phage-based CV Job Submission Summary ===")
    ...
    print("\nMonitor with:")
    print("  squeue -u $USER")
    print("  squeue -u $USER --name=bootstrap_strain_phage_cv  # Just bootstrap jobs")
    print("  tail -f logs/bootstrap_strain_phage_cv_*.out")
    print(f"\nResults will be in:")
    ...
    print(f"  Data retention: {args.output_dir}/data_retention_stats.csv")
    print(f"\nNote: This performs STRAIN+PHAGE-based cross-validation:")
    print(f"  - Each iteration uses 90% strains + 90% phages for training")
    print(f"  - Each iteration leaves out 10% strains + 10% phages for validation")
    print(f"  - Training data = training strains × training phages interactions only")
    print(f"  - Validation data = validation strains × validation phages interactions only")
    print(f"  - Validation strains and phages get features assigned using training clustering")
    print(f"  - Models predict interactions between validation strains × validation phages")
    print(f"  - Results show how well models generalize to completely novel strain-phage combinations")
    print(f"  - Expected data retention: ~81% (0.9 × 0.9) of original interactions per iteration")
