#!/usr/bin/env python3
"""
SLURM workflow submission for hybrid protein family (strains) + k-mer (phages) bootstrap cross-validation.
Creates a job array where each job handles one iteration of the bootstrap validation.
Based on slurm_bootstrap_submit.py but uses hybrid workflow.
"""
...
#SBATCH --output=logs/hybrid_bootstrap_%A_%a.out
#SBATCH --error=logs/hybrid_bootstrap_%A_%a.err
...
echo "CV Type: Strain-based with Protein Families (strains) + K-mers (phages)"
...
# Run the hybrid bootstrap iteration logic directly
...
# Step 1: Run hybrid workflow with modeling strains + all phages
...
if not os.path.exists(metrics_file):
    print('Running hybrid protein family + k-mer workflow...')
    
    # Import the hybrid workflow
    sys.path.append('{script_dir}')
    from hybrid_workflow import run_hybrid_protein_family_kmer_workflow
    
    # Create strain list file for modeling strains
    modeling_strain_list_path = os.path.join(iteration_output_dir, 'modeling_strains_for_workflow.csv')
    modeling_strain_df = pd.DataFrame({{'{args.strain_column}': modeling_strains}})
    modeling_strain_df.to_csv(modeling_strain_list_path, index=False)
    
    run_hybrid_protein_family_kmer_workflow(
        ...
        # Use modeling strains only for strain clustering
        strain_list=modeling_strain_list_path,
        phage_list='{args.interaction_matrix}',  # Use all phages from interaction matrix
    ...
    print('Modeling results already exist. Skipping hybrid workflow.')
...
# Step 3: Predict interactions for validation strains using all phage features
...
print('Running prediction workflow for validation strains...')
assign_predict_workflow(
    ...
    phage_feature_table_path=os.path.join(iteration_output_dir, 'phage', 'features', 'phage_final_feature_table.csv'),
...
"""
    ...
    script_path = os.path.join(run_dir, "hybrid_bootstrap_job_array.sh")
...
print('Aggregating results from {args.n_iterations} iterations (hybrid protein family + k-mer)...')
...
def main():
    parser = argparse.ArgumentParser(description="Submit hybrid protein family + k-mer bootstrap validation workflow as SLURM job array")
    ...
    # K-mer parameters for phages
    kmer_group = parser.add_argument_group('K-mer parameters (phages)')
    kmer_group.add_argument('--k', type=int, default=5, help="K-mer length for phages (default: 5).")
    kmer_group.add_argument('--one_gene', action='store_true', help="Include features with 1 gene for k-mers.")
    kmer_group.add_argument('--k_range', action='store_true', help="Generate range of k-mer lengths from 3 to k.")
    kmer_group.add_argument('--ignore_families', action='store_true', help="Ignore protein families in k-mer generation.")
    ...
    augmentation_group.add_argument('--use_augmentation', action='store_true', help="Enable data augmentation.")
    ...
    parser.add_argument('--mem_per_job', type=int, default=120, help='Memory per job in GB (default: 120).')
    parser.add_argument('--time_limit', default='36:00:00', help='Time limit per job (default: 36:00:00).')
    ...
    print(f"CV Type: Strain-based with Protein Families (strains) + K-mers (phages)")
    ...
    # Resource validation
    if args.mem_per_job < 64:
        print(f"WARNING: {args.mem_per_job}GB memory may be insufficient for hybrid workflow")
    ...
    print("Submitting hybrid bootstrap job array...")
    bootstrap_job_id = submit_job("hybrid_bootstrap_job_array.sh")
    ...
    print("\nMonitor with:")
    print("  squeue -u $USER")
    print("  squeue -u $USER --name=hybrid_bootstrap  # Just bootstrap jobs")
    print("  tail -f logs/hybrid_bootstrap_*.out")
    print(f"\nResults will be in:")
    ...
    print(f"\nNote: This performs hybrid strain-based cross-validation:")
    print(f"  - Each iteration uses 90% strains for training, 10% for validation")
    print(f"  - ALL phages used for both training and validation (no phage splitting)")
    print(f"  - Strain features: Protein families via MMseqs2 clustering")
    print(f"  - Phage features: K-mers of length {args.k}")
    print(f"  - Tests generalization to new strains with same phage feature space")
