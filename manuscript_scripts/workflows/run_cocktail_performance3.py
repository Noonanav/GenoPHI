...
def evalutate_cocktail_performance(
    ...
    if not full_predictions_df.empty:
        for (iteration, run_label), group in full_predictions_df.groupby(['iteration', 'modeling_method']):
            ...
            performance_path = os.path.join(bootstrap_dir, iteration, 'modeling', 'modeling_results', 'model_performance', 'model_performance_metrics.csv')
            ...
            feature_table_path = os.path.join(bootstrap_dir, iteration, 'modeling', 'feature_selection', 'filtered_feature_tables', f'select_feature_table_{max_cutoff}.csv')
            ...
            full_feature_table_path = os.path.join(modeling_dir, 'full_feature_table.csv')
            if os.path.exists(full_feature_table_path):
                full_features = pd.read_csv(full_feature_table_path)
            ...
            for n in [1, 3, 5]:
                ...
                if run_label == 'inverse_frequency_hierarchical_filter':   
                    # print('Running cocktail design analysis for:', iteration)
                    cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(group, full_features, clustering=False, clustering_method='hierarchical', display_intermediate=False, save_plot=False, top_n=n, generate_plot=False, plot_umap=False, default_cocktails=True)
                    cocktail_design_summary['clustering'] = 'None'
                    cocktail_design_summary['top_n'] = n
                    cocktail_design_summary['run_label'] = 'Default cocktail'
                    bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])

                    cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(group, full_features, clustering=True, clustering_method='hierarchical', display_intermediate=False, save_plot=False, top_n=n, generate_plot=False, plot_umap=False, default_cocktails=True)
                    cocktail_design_summary['clustering'] = 'hierarchical'
                    cocktail_design_summary['top_n'] = n
                    cocktail_design_summary['run_label'] = 'Default cocktail'
                    bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])

                    cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(group, full_features, clustering=True, clustering_method='HDBSCAN', display_intermediate=False, save_plot=False, top_n=n, generate_plot=False, plot_umap=False, split_by_strain=False, default_cocktails=True)
                    cocktail_design_summary['clustering'] = 'HDBSCAN'
                    cocktail_design_summary['top_n'] = n
                    cocktail_design_summary['run_label'] = 'Default cocktail'
                    bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])
